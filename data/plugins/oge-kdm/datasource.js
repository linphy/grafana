    ...
    return {
        ...
        execute: function() {
            GenericDatasource = (function () {
                /** @ngInject */
                function GenericDatasource(instanceSettings, $q, backendSrv, templateSrv, templateChange) {
                    this.$q = $q;
                ...
                GenericDatasource.prototype.query = function (options) {
                    ...
                    var that = this;
                    if (!query || !lodash_1.default.isArray(query.targets) || query.targets.length <= 0) {
                        return this.$q.when([]);
                    }
                    ...
                    for (var j = 0; j < query.targets.length; j++) {
                        ...
                        var queryInfo = query.targets[j].targetCode.indexOf('%') !== -1 || query.targets[j].targetCode.indexOf('json:') === 0;
                        if (queryInfo) {
                        ...
                        if (results && results.length > 0) {
                            for (var ind = 0; ind < results.length; ind++) {
                                if (results[ind].data) {
                                    ...
                                    var refIdName = void 0;
                                    for (var m = 0; m < query.targets.length; m++) {
                                        var kksC = results[ind].indexOf('_') > 0 ? results[ind].split('_')[0] : results[ind];
                ...
                GenericDatasource.prototype.addPoints = function (targets, attribute) {
                    ...
                    if (this.templateSrv.selectTargets && targets.length === 1 && targets[0][attribute] === '__shu-zhi-fen-xi') {
                        ...
                        var selectTargets = this.templateSrv.selectTargets;
                        for (var j = 0; j < selectTargets.length; j++) {
                ...
                //获取值
                GenericDatasource.prototype.getSpotDataValue = function (targets, query) {
                    var kksCodeName = '';
                    var promises = [];
                    var obj = this;
                    var paramsName;
                    ...
                    if (query.kdm && query.kdm.dataType === 2) {
                        //历史
                        var startT = query.range.from.valueOf();
                        var endT = query.range.to.valueOf();
                        paramsName = {
                ...
                GenericDatasource.prototype.getKksCodes = function (target) {
                    ...
                    return new Promise(function (resolve, reject) {
                        resolve(_this.backendSrv.datasourceRequest({
                            url: _this.url + '/kksInfo?method=searchKKSInfos&',
                            params: {
                                limit: 1000,
                                offset: 1,
                ...
                //数据处理
                GenericDatasource.prototype.resultsToGrafanaData = function (targets, resSpot, isAlert, isStep) {
                    ...
                    if (resSpot[0].data && resSpot[0].data.RTDataSets.length > 0) {
                        ...
                        for (var j = 0; j < resSpot[0].data.RTDataSets.length; j++) {
                            var arrRes = [];
                            ...
                            if (resSpot[0].data.RTDataSets[j].RTDataValues && rIndex != undefined) {
                                for (var i = 0; i < resSpot[0].data.RTDataSets[j].RTDataValues.length; i++) {
                                    if (isStep && (resSpot[0].data.RTDataSets[j].RTDataValues[i].Value === 922337180000000 || resSpot[0].data.RTDataSets[j].RTDataValues[i].Value > 922337180000000))
                                        continue;
                                    arrRes = [];
                            ...
                            if (isAlert) {
                                if (resSpot[1] && resSpot[1].data && resSpot[1].data.BasicResponse.succeeded === 1) {
                                    if (resSpot[1].data.Alarm === undefined || resSpot[1].data.Alarm.length === 0) {
                                        ...
                                        for (var m = 0; m < resSpot[1].data.Alarm.length; m++) {
                ...
                GenericDatasource.prototype.metricFindQueryPoint = function (options) {
                    var fieldsName;
                    var queryValue;
                    var limitCount;
                    var orderbyCount;
                ...
                GenericDatasource.prototype.mapToTextValue = function (result) {
                    if (result.data && result.data.KKSInfos && result.data.KKSInfos.length > 0) {
                        var arr = [];
                        for (var i = 0; i < result.data.KKSInfos.length; i++) {
                            if (result.data.KKSInfos[i].facilityF0) {
                                //机组
                                var textVal = result.data.KKSInfos[i].facilityF0Value;
                                var valVal = result.data.KKSInfos[i].facilityG + '' + result.data.KKSInfos[i].facilityF0;
                                if (result.data.KKSInfos[i].facilityF0 == 0 && valVal == 'A0')
                                    continue;
                    ...
                    //kks标签
                    return lodash_1.default.map(result.data.KKSInfos, function (d, i) {
                        return { text: d, value: i };
                    });
                };
                GenericDatasource.prototype.metricFindQueryWave = function (options) {
                    var _this = this;
                    if (options === "jz") {
                        var dc = this.templateSrv.replace("$dc");
                        if (dc === "$dc") {
                    ...
                    return this.getLoginStatus().then(function (data) {
                        ...
                        return _this.request(options1).then(_this.mapToTextValueWave);
                    });
                };
                GenericDatasource.prototype.mapToTextValueWave = function (result) {
                ...
                GenericDatasource.prototype.getLoginStatus = function () {
                    var deferred = this.$q.defer();
                    deferred.resolve(true);
                    return deferred.promise;
                };
                GenericDatasource.prototype.buildRequestUrl = function (relativePath) {
                    return this.url + "/wave" + relativePath;
                };
                GenericDatasource.prototype.request = function (rawOptions) {
                ...
                GenericDatasource.prototype.buildQueryParameters = function (options) {
                    ...
                    if (kdm.pointOrWave && kdm.pointOrWave === 1) {
                        ...
                        targets = lodash_1.default.map(options.targets, function (target) {
                            return {
            ...
            exports_1("default", GenericDatasource);
        }
    }
});
