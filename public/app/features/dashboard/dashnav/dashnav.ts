...
export class DashNavCtrl {
  ...
  /** @ngInject */
  constructor(private $scope, private dashboardSrv, private $location, public playlistSrv, public $interval) {
    appEvents.on('save-dashboard', this.saveDashboard.bind(this), $scope);
    $scope.nowTime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
    $interval(() => {
      $scope.nowTime = moment(new Date()).format('YYYY-MM-DD HH:mm:ss');
    }, 1000);
