...
export class DashboardModel {
  ...
  constructor(data, meta?) {
    ...
    this.bgColor = data.bgColor || '#003366';
  ...
  getNextQueryLetter(panel) {
    ...
    var refId = _.find(letters, function(refId) {
      return _.every(panel.targets, function(other) {
        return other.refId !== refId;
      });
    });

    if (panel.targets.length >= 26 && refId === undefined) {
      refId = '' + panel.targets.length;
    }
    return refId;
  }
