...
const END_ROW = '\r\n';
const QUOTE = '';
const EXPORT_FILENAME = 'kam_data_export.csv';
...
export function saveSaveBlob(payload, fname) {
  let exportContent = '\uFEFF';
  const blob = new Blob([exportContent + payload], { type: 'text/plain;charset=utf-8' });
  saveAs(blob, fname);
}

export function exportTableDataToCsvByName(pname, table) {
  const payload = convertTableDataToCsv(table, false);
  let exportContent = '\uFEFF';
  const blob = new Blob([exportContent + payload], { type: 'text/plain;charset=utf-8' });
  let fname = pname + '.csv';
  saveAs(blob, fname);
