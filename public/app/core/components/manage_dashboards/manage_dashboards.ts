...
export class ManageDashboardsCtrl {
  ...
  starredFilterOptions = [{ text: '过滤标星仪表盘', disabled: true }, { text: '是' }, { text: '否' }];
  ...
  delete() {
    ...
    if (folderCount > 0 && dashCount > 0) {
      text += `已选择的文件夹${folderCount === 1 ? '' : 's'} 和仪表盘${dashCount === 1 ? '' : 's'}?`;
      text2 = `在文件夹${folderCount === 1 ? '' : 's'} 下的所有仪表盘也将会被删除`;
    } else if (folderCount > 0) {
      text += `已选择的文件夹${folderCount === 1 ? '' : 's'} 和所有文件夹下的仪表盘?`;
    } else {
      text += `已选择的仪表盘${dashCount === 1 ? '' : 's'}?`;
    }
