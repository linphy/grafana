...
export class ElasticDatasource {
  ...
  query(options) {
    ...
    let target;
    const sentTargets = [];
    let replaceWord = this.timeSrv; // [OGE]
    let templateSrvVal = this.templateSrv; // [OGE]
    ...
    return this.post('_msearch', payload).then(function(res) {
      return new ElasticResponse(sentTargets, res, replaceWord, templateSrvVal).getTimeSeries();
    });
  }
