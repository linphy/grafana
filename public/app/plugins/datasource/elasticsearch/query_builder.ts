...
export class ElasticQueryBuilder {
  ...
  getDateHistogramAgg(aggDef) {
    ...
    esAgg.offset = settings.offset || '-8h'; //[OGE]
    esAgg.order = { _key: settings.order || 'asc' }; //[OGE]
  ...
  build(target, adhocFilters?, queryString?) {
    ...
    for (i = 0; i < target.metrics.length; i++) {
      ...
      if (metric.type === 'bucket_script') {
        let bool = false;
        if (metric.ParamNames === undefined) {
          continue;
        } else {
          for (let j = 0; j < metric.ParamNames.length; j++) {
            if (metric.ParamNames[j].text && /^\d*$/.test(metric.ParamNames[j].text)) {
              bool = true;
              continue;
            }
            bool = false;
          ...
          if (bool) {
            let ParamsBuckets_path = { var_1: '' };
            ParamsBuckets_path.var_1 = metric.pipelineAgg;
            for (let z = 0; z < metric.ParamNames.length; z++) {
              ParamsBuckets_path['var_' + (z + 2)] = metric.ParamNames[z].text;
            }
            metricAgg = { buckets_path: ParamsBuckets_path };
          } else {
            continue;
          }
        }
      } else if (queryDef.isPipelineAgg(metric.type)) {
        //END
