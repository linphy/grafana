...
export class OpenTsQueryCtrl extends QueryCtrl {
  ...
  addTag() {
    if (this.target.filters && this.target.filters.length > 0) {
      this.errors.tags = '请删除筛选器以使用标记，标记和筛选器是互斥的。';
    }
  ...
  addFilter() {
    if (this.target.tags && _.size(this.target.tags) > 0) {
      this.errors.filters = '请删除标记以使用筛选器，标记和筛选器是互斥的。';
    }
  ...
  validateTarget() {
      ...
      try {
        if (this.target.downsampleInterval) {
          ...
          errs.downsampleInterval =
            "You must supply a downsample interval (e.g. '1m' or '1h').必须提供降低采样间隔（例如'1m'或'1h')。";
        }
      } catch (err) {
    ...
    if (this.target.tags && _.has(this.target.tags, this.target.currentTagKey)) {
      errs.tags = "重复标记键 '" + this.target.currentTagKey + "'.";
    }
