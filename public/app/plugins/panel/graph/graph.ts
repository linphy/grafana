...
class GraphElement {
  ...
  buildFlotPairs(data) {
    for (let i = 0; i < data.length; i++) {
      ...
      if (this.panel.lines && this.panel.smoothLine && series.data.length > 0) {
        series.data = this.unLinearRegression(series.data);
      }
  ...
  unLinearRegression(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      //因为时间戳数值过大，将x轴的值设置为1，2，3，4,...,n
      xsum1 += i + 1;
      xsum2 += Math.pow(i + 1, 2);
      xsum3 += Math.pow(i + 1, 3);
      xsum4 += Math.pow(i + 1, 4);
      ysum1 += data[i][1];
      ysum2 += data[i][1] * (i + 1);
      ysum3 += data[i][1] * Math.pow(i + 1, 2);
    }
    //根据以下算式算出未知系数a0,a1,a2
    // ysum1 = a0*l+a1*xsum1+a2*xsum2;
    // ysum2 = a0*xsum1+a1*xsum2+a2*xsum3;
    // ysum3 = a0*xsum2+a1*xsum3+a2*xsum4;
    var fenzi =
      (ysum2 * l - xsum1 * ysum1) * (xsum3 * l - xsum1 * xsum2) -
      (ysum3 * l - xsum2 * ysum1) * (xsum2 * l - xsum1 * xsum1);
    var fenmu =
      (xsum3 * l - xsum1 * xsum2) * (xsum3 * l - xsum1 * xsum2) -
      (xsum2 * l - xsum1 * xsum1) * (xsum4 * l - xsum2 * xsum2);
    a2 = fenzi / fenmu;
    a1 = (ysum3 * l - xsum2 * ysum1 - (xsum4 * l - xsum2 * xsum2) * a2) / (xsum3 * l - xsum1 * xsum2);
    a0 = ysum1 / l - xsum1 / l * a1 - xsum2 / l * a2;
    var newXY = [];
    for (var j = 0; j < data.length; j++) {
      // var y = a0+a1*data[j][0]+data[j][0]*data[j][0]*a2;
      var y = a0 + a1 * (j + 1) + (j + 1) * (j + 1) * a2;
      newXY.push([data[j][0], y]);
    }
    return newXY;
  }
  ...
  prepareXAxis(options, panel) {
    switch (panel.xaxis.mode) {
      case 'series': {
        options.series.bars.barWidth = panel.barsWidth || 0.7;
